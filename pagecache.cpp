...
int main(int argc, char** argv)
{
	...
	void* mapped_mem = mmap(NULL, stat_buffer.st_size, PROT_READ, MAP_FILE | MAP_PRIVATE, fd, 0);
	if (mapped_mem == MAP_FAILED) { perror("mmap() failed"); return 1; }

	int page_size = getpagesize();
	int num_pages = stat_buffer.st_size / page_size + (stat_buffer.st_size % page_size == 0 ? 0 : 1);
	char* page_vector = (char*) malloc(num_pages);
	...
	int total_in_mem = 0;
	printf("Page cache for '%s' (file size=%lld, page size=%d):\n", argv[1], stat_buffer.st_size, page_size);
	for (int i = 0; i < num_pages; i++) {
		bool in_mem = page_vector[i] & 1;
